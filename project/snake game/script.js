...
const random = () => {
    ...
    if (isLargeDevice <= 500) {
        ...
        document.querySelector(".area").setAttribute("style", `height: 520px;width: 450px;background-image: url(./basket/red_dot.jpg), url(./basket/black_back.jpg);background-size: 10px 10px, auto;background-position: ${random_width}px ${random_height}px, center center;background-repeat: no-repeat, no-repeat;display: flex;position: relative;z-index: 1;`);
        console.log(random_width, random_width);
    }
}
...
const increase = () => {
    ...
    Div.innerHTML = `<div class="are"><div style="background-color: green; width: 10px; height: 10px; margin-top: ${arrt1-10*num}px; margin-left: ${arrl1-10*num}px"></div></div>`;
    Div.setAttribute("class", `tail${num}`);

    num = 1 + num;

    Div.setAttribute("style", `height: 520px;width: 450px;display: flex;position: absolute;top: 183px;z-index:${num};`);

    document.querySelector(`.tails`).append(Div);
